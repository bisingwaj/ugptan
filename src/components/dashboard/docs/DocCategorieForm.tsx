...
/**
 * Formulaire d'une catégorie documentaire.
 *
 * Jumeau de `EvtCategorieForm` sans en être une variante : les deux écrivent
...
import {
  enregistrerCategorieDocAction,
  supprimerCategorieDocAction,
  type DocFormState,
...
export type CategorieDocItem = {
  ...
  color: string | null;
  position: number;
  /** Nombre de documents rattachés — affiché sur le bouton de suppression. */
  usage: number;
};

export function DocCategorieForm({ item }: { item?: CategorieDocItem }) {
  const t = ADMIN_DOCS;
  const creation = !item;

  const [state, formAction, pending] = useActionState(enregistrerCategorieDocAction, etatInitial);
  ...
  const idBase = useId();
  const idSuppression = `suppr-cat-doc-${item?.id ?? "aucune"}`;
