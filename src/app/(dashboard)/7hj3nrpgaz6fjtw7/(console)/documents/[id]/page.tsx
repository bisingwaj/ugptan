...
export default async function ModifierDocumentPage(props: {
  ...
  const [document, referentiels] = await Promise.all([
    chargerDocument(id),
   ...
   */
  const publicUrl = enLigne ? `/fr${NAV.ressources}?doc=${document.id}` : null;
    ...
    <>
      ...
      <div style={{ marginTop: 26 }}>
        {document.fichier ? (
          ...
          <div className="adm-panel"><p className="adm-hint">{t.fichierAucun}</p></div>
        )}
      </div>
