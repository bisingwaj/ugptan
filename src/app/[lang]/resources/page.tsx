...
export async function generateMetadata(props: { params: Promise<{ lang: string }> }): Promise<Metadata> {
  ...
  return {
    ...
    description: t.lead,
    alternates: {
      canonical: `/${lang}${NAV.ressources}`,
      languages: { fr: `/fr${NAV.ressources}`, en: `/en${NAV.ressources}` },
    },
    openGraph: {
      title: `${t.titre} · UGPTN`,
      description: t.lead,
      url: `${SITE_URL}/${lang}${NAV.ressources}`,
      type: "website",
    },
  };
}
...
function lien(lang: string, params: Recherche, changement: Partial<Recherche>): string {
  ...
  const base = `/${lang}${NAV.ressources}`;
      ...
      <section style={{ padding: "clamp(40px,5vw,60px) var(--pad-x) clamp(64px,8vw,110px)" }}>
        <div className="section__inner">
          ...
          <div className="doc-compte">
            ...
            {filtre && (
              <Link href={route(lang, NAV.ressources)} className="actu-avis__lien" scroll={false}>
                {r.reset}
              </Link>
            )}
          </div>
